...
const code = fs.readFileSync('modules/nVideo/lib/index.js', 'utf8');
...
let newCode = code.replace(/let nativeBinding = null;/, `let nativeBinding = null;\n${replacement}`);
newCode = newCode.replace(/transcode\(inputPath, outputPath, opts\) \{[\s\S]*?return result;\n    \},/, transcodeReplacement.trim() + ',');

fs.writeFileSync('modules/nVideo/lib/index.js', newCode);
console.log('done');
